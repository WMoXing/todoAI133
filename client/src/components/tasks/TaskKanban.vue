...
function columnTasks(status) {
  return taskStore.getTasksByStatus(status).filter(t => !t.parentId)
}
...
function doneCount(task) {
  return (childMap.value[task.id] || []).filter(c => c.completed).length
}

function toggleChild(child, val) {
  taskStore.editTask(child.id, { completed: val, status: val ? 'done' : 'todo' })
}
...
async function handleDrop(e, status) {
  if (draggedTask && draggedTask.status !== status) {
    await taskStore.editTask(draggedTask.id, { status })
  }
  draggedTask = null
}
